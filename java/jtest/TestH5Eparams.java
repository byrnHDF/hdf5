...
public class TestH5Eparams {
    ...
    {
        try {
            String class_name = H5Eget_class_name(hdf_java_classid);
            assertNotNull("H5Eget_class_name: " + class_name, class_name);
            assertEquals("H5Eget_class_name: ", "HDF-Java-Error", class_name);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Eget_class_name: " + err);
        }
    }
    ...
    {
        H5.H5Ecreate_msg(hdf_java_classid, H5E_MAJOR(), null);
    }
    ...
    {
        try {
            long err_id = H5.H5Ecreate_msg(hdf_java_classid, H5E_MAJOR(), "Error in Test");
            assertFalse("H5Ecreate_msg_major: " + err_id, err_id < 0);
            H5Eclose_msg(err_id);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Ecreate_msg_major: " + err);
        }
    }
    ...
    public void testH5Ecreate_msg_minor()
    {
        try {
            ...
            assertFalse("H5Ecreate_msg_minor: " + err_id, err_id < 0);
            H5Eclose_msg(err_id);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Ecreate_msg_minor: " + err);
        }
    }
    ...
    public void testH5Eget_msg()
    {
        ...
        String msg           = null;
        try {
            err_id = H5.H5Ecreate_msg(hdf_java_classid, H5E_MAJOR(), "Error in Test");
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Eget_msg: " + err);
        }
        assertFalse("H5Eget_msg: H5Ecreate_msg - " + err_id, err_id < 0);
        try {
            msg = H5Eget_msg(err_id, error_msg_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Eget_msg: " + err);
        ...
        assertEquals("H5Eget_msg: ", "Error in Test", msg);
    ...
    public void testH5Ecreate_stack()
    {
        long stk_id = -1;
        try {
            stk_id = H5Ecreate_stack();
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Ecreate_stack: " + err);
        }
        assertFalse("H5Ecreate_stack: " + stk_id, stk_id < 0);
        try {
    ...
    public void testH5Eauto_is_v2()
    {
        boolean is_v2 = false;
        try {
            is_v2 = H5Eauto_is_v2(current_stackid);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Eget_num()
    {
        ...
        assertTrue("H5Eget_num", num_msg == 0);
    }
